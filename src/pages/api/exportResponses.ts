...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const saveResponsesIds: string[] = await UserController.getSavedResponses(userID);
        if (!saveResponsesIds) {
            throw {
                code: 204,
        ...
        const savedResponses: ResponseController[] = await ResponseController.getResponsesByIds(saveResponsesIds);
        if (!savedResponses) {
            throw {
                code: 204,
